...
class SFTDataset(Dataset):
    ...
    def __init__(
        ...
        self.weight_key = getattr(self.strategy.args, "score_key", None)
        self.reward_key = 'Skywork/Skywork-Reward-Gemma-2-27B-v0.2'
    ...
    def process_data(self, data):
        ...
        if not self.pretrain_mode:
            ...
            prompt_ids_len = 0
        if self.weighted:
            return {"prompt": prompt, "response": response, "prompt_ids_len": prompt_ids_len, "weight": float(data[self.weight_key])}
        else:
    ...
    def __getitem__(self, idx):
        ...
        response = self.responses[idx][-1]["content"]
        if not self.pretrain_mode:
        ...
        if self.weighted:
            return prompt_ids_len, input_token["input_ids"], input_token["attention_mask"], info, self.weights[idx]
    ...
    def collate_fn(self, item_list):
        ...
        if self.weighted:
            for prompt_ids_len, input_id, attention_mask, info, weight in item_list:
        ...
        if self.weighted:
            return prompt_ids_lens, input_ids, attention_masks, infos, weights
