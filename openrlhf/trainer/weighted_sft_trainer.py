...
class WeightedSFTTrainer(ABC):
    ...
    def __init__(
        ...
        self.loss_fn = GPTLMLoss(reduction="none")
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            self.model.train()
            loss_mean = 0
            for prompts_id_lens, inputs, attention_masks, infos, weights in self.train_dataloader:
                ...
                #print(gpt_loss.shape)
                gpt_loss = gpt_loss.reshape(output.logits.shape[0], -1) # this is batch_size
                #print(gpt_loss.shape) # this is batch_size * seq_len
                #print(weights.shape) # this is batch_size
                # no gradient on weights, convert to tensor
                
                weights = torch.tensor(weights, requires_grad=False).to(torch.cuda.current_device())
                ...
                gpt_loss = torch.sum(gpt_loss * weights) 
                #gpt_loss = torch.sum(gpt_loss * weights) / torch.sum(weights)
